...
{
#if 1    
    NPI_InitTransportEx(simpleBLE_NpiSerialCallback, 0, 
...
#define TEMP_COEFF 4.5  
    ...
    else if((length == 9) && str_cmp(pBuffer, "AT+MCU", 6))
    {
        uint8 ch = pBuffer[6];  // AT+ADC的尾号，例如AT+ADC4中的4
...
void simpleBLE_HandleKeys(uint8 keys)
{
    ...
    if ( keys & HAL_KEY_SW_6 )
    {
        ...
        */
        ...
        NPI_WriteTransport_ln((uint8*)str, strlen(str)); // 通过串口发送出来
...
bool simpleBLE_MT_CMD_Handle(uint8 *pBuffer, uint16 length)
{
    ...
    */
    ...
    if((length >= 8) && str_cmp(pBuffer, "AT+MCU+", 7))
    {
        ...
        //NPI_WriteTransport(pBuffer,length);
        // 格式化
        sprintf(strTemp, "AT+BTN%d#", btnId);
        NPI_WriteTransport((uint8*)strTemp, strlen(strTemp));
        #if 0
    ...
    else if( (length >= 9) && str_cmp(pBuffer, "AT+LED2=", 8))
    {
        onoff = pBuffer[8];
        if(onoff == '0')
        {
