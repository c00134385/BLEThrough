...
uint16 SimpleBLEPeripheral_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  //sprintf(printbuf, "OK:%d\r\n", count);
  //NPI_PrintString(printbuf); 
  ...
  {      
    ...
    sprintf(printbuf, "PWRMGR_ALWAYS_ON:%d\r\n", count);
    NPI_PrintString(printbuf); 
    ...
    //            simpleBLEChar6DoWrite2 = FALSE;
        ...
        // 目前有问题， 不要启用
                ...
                if(pReport.pValue != NULL)
                {
                    ...
                    //NPI_WriteTransport("\r\nnty:", 6);
                    //NPI_WriteTransport(pReport.pValue, pReport.len); 
